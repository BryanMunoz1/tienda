...
class ProductoCarrito:
    def __init__(self, codigoproducto, cantidadproducto, precioproducto):
        self.__codigoproducto = codigoproducto
...
class CarritoCompra:
    ...
    def comprarproducto(self, productos):
        Producto.mostrarproductos()  # Mostrar la tabla de productos antes de comprar
        codigo = input(Fore.WHITE + "Ingrese el código del producto a comprar: " + Style.RESET_ALL)
        producto = next((p for p in productos if p.getcodigoproducto() == codigo), None)
        if producto:
            try:
                cantidad = int(input(Fore.WHITE + f"Ingrese la cantidad (Disponible: {producto.getinventarioproducto()}): " + Style.RESET_ALL))
            except ValueError:
                print(Fore.RED + "Debe ingresar un número válido." + Style.RESET_ALL)
                return
            if cantidad > producto.getinventarioproducto():
                print(Fore.RED + "No hay suficiente inventario." + Style.RESET_ALL)
                return
            
            subtotal = cantidad * producto.getprecioproducto()
            nuevo_producto = ProductoCarrito(producto.getcodigoproducto(), cantidad, producto.getprecioproducto())
            self.agregarproducto(nuevo_producto)

            # Confirmación de compra
            confirmacion = input(Fore.WHITE + f"¿Está seguro de comprar {cantidad} unidades de {producto.getnombreproducto()}? (s/n): " + Style.RESET_ALL).lower()
            if confirmacion == 's':
                print(Fore.GREEN + f"Producto {producto.getnombreproducto()} añadido al carrito." + Style.RESET_ALL)
            else:
    ...
    def facturar(self):
        ...
        print(Fore.GREEN + "+-------------------+----------------+----------------+----------------+")
        print(Fore.WHITE + "Código Producto\tNombre\tCantidad\tSubtotal")
        print(Fore.GREEN + "+-------------------+----------------+----------------+----------------+")
        ...
        for producto in self.__productoscarrito:
            ...
            total += subtotal
            print(Fore.WHITE + f"{str(producto.getcodigoproducto()).zfill(3)}\t\t{producto.getnombreproducto()}\t{producto.getcantidadproducto()}\t${subtotal:.2f}")
...
def digitardato(mensaje):
    print(Fore.WHITE + mensaje + Style.RESET_ALL)
    while True:
        try:
            char = msvcrt.getch().decode('utf-8')
            if ord(char) == 27:  # ESC para salir
                break
        except Exception as e:
...
def mostrar_menu():
    ...
    while True:
        ...
        if opcion == '1':
            ...
            carrito.comprarproducto(Producto.getlistaproductos())  # Pasar la lista de productos
            input(Fore.WHITE + "Presione Enter para continuar..." + Style.RESET_ALL)
        elif opcion == '6':
